...
```mermaid
graph LR
    A["Umbraco Content<br/>(stagePage)"] -->|blueprintKey| B["StagePageController<br/>(GET/POST handler)"]
...
> 🔵 **Prism Platform** — Prism provides a content type generator. Run this once to create the `stagePage` content type in your Umbraco backoffice.

In Umbraco's backoffice:
1. **Content** → **Content Types**
2. Look for **`stagePage`** — if it exists, skip to Step 2
3. If not, run the Prism seeding command (see ASPIRE_DEV.md for details)

The `stagePage` content type has these properties:
...
1. Go to **Content**
2. Click **+ Create**
3. Choose **`stagePage`** as the content type
...
Umbraco automatically route-hijacks the `stagePage` content type with the `StagePageController` (provided by Prism). When you visit the page:
...
- [ ] Create `stagePage` content type in Umbraco (auto-generated by Prism)
- [ ] Create a content node with `blueprintKey` configured
- [ ] Implement minimal `StagePageController` (or use base class as-is)
- [ ] Publish and test
