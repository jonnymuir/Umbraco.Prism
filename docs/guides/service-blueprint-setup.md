...
```mermaid
graph LR
    A["Umbraco Content<br/>(touchpointPage)"] -->|blueprintKey| B["TouchpointPageController<br/>(GET/POST handler)"]
...
> 🔵 **Prism Platform** — Prism provides a content type generator. Run this once to create the `touchpointPage` content type in your Umbraco backoffice.

In Umbraco's backoffice:
1. **Content** → **Content Types**
2. Look for **`touchpointPage`** — if it exists, skip to Step 2
3. If not, run the Prism seeding command (see ASPIRE_DEV.md for details)

The `touchpointPage` content type has these properties:
...
1. Go to **Content**
2. Click **+ Create**
3. Choose **`touchpointPage`** as the content type
...
Umbraco automatically route-hijacks the `touchpointPage` content type with the `TouchpointPageController` (provided by Prism). When you visit the page:
...
public class TouchpointPageController(
    ILogger<TouchpointPageController> logger,
...
- [ ] Create `touchpointPage` content type in Umbraco (auto-generated by Prism)
- [ ] Create a content node with `blueprintKey` configured
- [ ] Implement minimal `TouchpointPageController` (or use base class as-is)
- [ ] Publish and test
