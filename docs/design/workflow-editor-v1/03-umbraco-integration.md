...
This section owns how the V1 workflow editor fits into Umbraco v17 and the surrounding hosts: public, member, and business-app back-stage surfaces. The README frames the system simply as workflow editor, workflow engine, and forms engine. This document stays focused on the Umbraco-specific responsibilities behind that framing: Document Types, route-hijacking controllers, backoffice hosting, auth and roles enforcement, and the constraints that keep the TestSite's existing contract intact as editor work lands.

**Note on runtime surfaces:** The public and member surfaces use the **Forms Engine** (Prism's rendering layer) to display workflow shells. The back-stage surface uses the **Workflow Engine** (MockBusinessApp runtime) to manage instance state and transitions. Both consume the same projected `WorkflowDefinitionFile`.

---

## 2. Surface Model

The workflow editor publishes one workflow definition that the workflow engine then uses across three distinct runtime surfaces. Each surface maps onto the same authored workflow through a different entry point and authentication context.
...
- **Same published file:** the `WorkflowDefinitionFile` generated by the editor's publish step is consumed by MockBusinessApp's workflow engine unchanged.
...
This rule applies equally to the `WorkflowLandingController` (if added) and to any future agent-support redirect helpers.
...
The published `WorkflowDefinitionFile` is consumed by the MockBusinessApp's workflow engine at seed time from `workflow-seeds/*.json`. For V1, the workflow editor's deterministic publish step writes to this directory (or a configured output path). The MockBusinessApp reads seeds on startup; no live reload mechanism is required in V1.
...
*This document is scoped to the Umbraco integration layer. The editor model, publish contract, and optional Copilot or MCP support are covered in sibling sections of this design.*
