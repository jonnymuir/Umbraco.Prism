...
/// without needing a real agent run. Mirrors <see cref="JugglingLicenceCmsServiceBlueprintSeeder"/>
...
/// </remarks>
public class LicenceTransferCmsServiceBlueprintSeeder(
    IContentService contentService,
    IContentTypeService contentTypeService,
    IServiceBlueprintSourceStore workflowSourceStore,
    IWebHostEnvironment env,
    IRuntimeState runtimeState,
    ILogger<LicenceTransferCmsServiceBlueprintSeeder> logger)
    : INotificationAsyncHandler<UmbracoApplicationStartedNotification>
{
    ...
    private void EnsureContentPageUnderHome()
    {
        ...
        if (contentType == null)
        {
            logger.LogDebug("LICENCE TRANSFER SEEDER: cmsServiceRequestPage doc type not found; skipping (run again after the content-type seeder)");
            return;
        }
        ...
        var page = contentService.Create(
            TestSiteSeedContract.LicenceTransferPageName, homePage.Id, TestSiteSeedContract.CmsServiceRequestPageAlias);
