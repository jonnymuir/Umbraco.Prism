...
/// </summary>
public class TouchpointPageSeeder(
    IContentService contentService,
    IContentTypeService contentTypeService,
    IWebHostEnvironment env,
    IRuntimeState runtimeState,
    ILogger<TouchpointPageSeeder> logger)
    : INotificationAsyncHandler<UmbracoApplicationStartedNotification>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(ex, "TOUCHPOINT PAGE SEEDER: Unexpected error; skipping");
        }
    ...
    private void CleanupOldRetirementQuotePage()
    {
        var oldPage = EnumerateContentTree()
            .FirstOrDefault(c => c.ContentType.Alias == TestSiteSeedContract.TouchpointPageAlias
        ...
        if (oldPage != null)
        {
            logger.LogInformation("TOUCHPOINT PAGE SEEDER: Deleting old 'Retirement Quote' node (id={Id})", oldPage.Id);
            var deleteResult = contentService.Delete(oldPage);
            if (deleteResult.Success)
                logger.LogInformation("TOUCHPOINT PAGE SEEDER: Old demo node deleted successfully");
            else
                logger.LogWarning("TOUCHPOINT PAGE SEEDER: Delete failed — {Reason}", deleteResult.Result);
        }
    }
    ...
    private void EnsureHomeAndDashboard()
    {
        ...
        if (homeType == null)
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: homePage doc type not found; skipping home/dashboard seed");
            return;
        }
        ...
        if (homePage == null)
        {
            logger.LogInformation("TOUCHPOINT PAGE SEEDER: Creating seeded home page");
        ...
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: memberDashboard doc type not found; skipping dashboard seed");
            return;
        }
...
#pragma warning restore CS0618
        ...
        {
            logger.LogInformation("TOUCHPOINT PAGE SEEDER: Creating seeded dashboard page");
    ...
    private void EnsureTouchpointPageUnderHome(string blueprintKey, string pageName, string label)
    {
        var homePage = TestSiteSeedContract.FindContentByAlias(contentService, TestSiteSeedContract.HomePageAlias);
        if (homePage == null)
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: homePage not found; skipping {Label}", label);
            return;
        }

        var contentType = contentTypeService.Get(TestSiteSeedContract.TouchpointPageAlias);
        if (contentType == null)
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: touchpointPage doc type not found; skipping {Label}", label);
            return;
        }

        var existing = EnumerateContentTree()
            .FirstOrDefault(content =>
                content.ContentType.Alias == TestSiteSeedContract.TouchpointPageAlias
        ...
        if (existing != null && existing.ParentId != homePage.Id)
        {
            logger.LogInformation(
                "TOUCHPOINT PAGE SEEDER: Replacing workflow page {BlueprintKey} at parent {ParentId} so the seeded route stays under Home",
                blueprintKey,
                existing.ParentId);
        ...
        logger.LogInformation("TOUCHPOINT PAGE SEEDER: Creating {Label}", label);

        var page = contentService.Create(pageName, homePage.Id, TestSiteSeedContract.TouchpointPageAlias);
    ...
    private void EnsureServiceRequestHubPage()
    {
        ...
        if (homePage == null)
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: homePage not found; skipping workflow hub seed");
            return;
        }
        ...
        if (contentType == null)
        {
            logger.LogDebug("TOUCHPOINT PAGE SEEDER: serviceRequestHub doc type not found; skipping (run again after seeder)");
            return;
        }
...
#pragma warning restore CS0618
        ...
        {
            logger.LogInformation(
                "TOUCHPOINT PAGE SEEDER: Replacing workflow hub at parent {ParentId} so the seeded route stays under Home",
                existing.ParentId);
            var deleteResult = contentService.Delete(existing);
            if (!deleteResult.Success)
            {
        ...
        logger.LogInformation("TOUCHPOINT PAGE SEEDER: Creating seeded workflow hub");
...
#pragma warning restore CS0618
        ...
        string label)
    {
        ...
        {
            logger.LogWarning("TOUCHPOINT PAGE SEEDER: Save failed — {Reason}", saveResult.Result);
            return;
        }
        ...
        {
            logger.LogInformation("TOUCHPOINT PAGE SEEDER: {Label} published (id={Id}, changed={Changed})", label, content.Id, changed);
        }
        else
            logger.LogWarning("TOUCHPOINT PAGE SEEDER: Publish failed — {Reason}", publishResult.Result);
    }
}
