...
/// <summary>
/// Seeds the "Apply for a juggling licence" public service request demo — the reference example
/// proving TestSite's backoffice-authored, uSync-portable, in-Umbraco-only implementation (via
/// Wayfinder.Umbraco's own "Wayfinder" backoffice section). Mirrors <see cref="StagePageSeeder"/>'s
/// convention exactly: C# seeders own initial demo data; uSync only captures subsequent portable
...
/// <c>UmbracoProcessManagerEngine</c> is a singleton that loads its definitions once at construction, so a
...
/// <c>UmbracoServiceBlueprintStore</c>'s own remarks) — a raw DB write bypasses that
/// entirely regardless of source (seeder, migration, manual query).
/// </remarks>
public class JugglingLicenceServiceRequestSeeder(
    IContentService contentService,
    IContentTypeService contentTypeService,
    IServiceBlueprintSourceStore workflowSourceStore,
    IWebHostEnvironment env,
    IRuntimeState runtimeState,
    ILogger<JugglingLicenceServiceRequestSeeder> logger)
    : INotificationAsyncHandler<UmbracoApplicationStartedNotification>
{
    ...
    private async Task EnsureDefinitionSeededAsync(CancellationToken cancellationToken)
    {
        ...
        var path = Path.Combine(env.ContentRootPath, "service-blueprints", "apply-for-a-juggling-licence.json");
        if (!File.Exists(path))
        {
    ...
    private void EnsureContentPageUnderHome()
    {
        ...
        if (contentType == null)
        {
            logger.LogDebug("JUGGLING LICENCE SEEDER: publicServiceRequestPage doc type not found; skipping (run again after the content-type seeder)");
            return;
        }

        // Filtered by blueprintKey, not just alias — a second publicServiceRequestPage instance now exists
        ...
        logger.LogInformation("JUGGLING LICENCE SEEDER: Creating seeded service request page");

        var page = contentService.Create(
            TestSiteSeedContract.JugglingLicencePageName, homePage.Id, TestSiteSeedContract.ServiceRequestPageAlias);
