...
{
    public static string GetRequiredString(WorkflowActionDefinition action, string key)
    {
    ...
    public static string? GetOptionalString(WorkflowActionDefinition action, string key) =>
        action.Parameters[key]?.GetValue<string>();

    public static bool GetBoolean(WorkflowActionDefinition action, string key, bool defaultValue = false) =>
...
public sealed class FormsLoadWorkflowActionHandler : WorkflowActionHandlerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(WorkflowActionExecutionResult.Success(
            $"Loaded forms definition '{formDefinitionId}' for state '{context.TargetState.StateKey}'.",
