...
/**
 * Editor rendering tests for the three workflows migrated to the new
 * queues/gateways/routes format: planning, community-enquiry, information-request.
 *
 * These prove that hydrateWorkflowDefinition correctly normalises the new
...
function graphStoryUrl(storyName: string): string {
  return `/iframe.html?id=workflow-editor-workflow-graph--${storyName}&viewMode=story`;
}

// ─── Planning Application (migrated) ─────────────────────────────────────────

test.describe('Planning workflow — migrated format', () => {
...
test.describe('Community Enquiry workflow — migrated format', () => {
...
test.describe('Information Request workflow — migrated format', () => {
