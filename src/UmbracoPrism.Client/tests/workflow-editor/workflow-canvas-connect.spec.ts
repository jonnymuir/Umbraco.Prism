...
async function sourceHandleCentre(page: Page, stageKey: string) {
  return page.locator('prism-workflow-graph').evaluate((graphElement, key) => {
...
test.describe('Workflow canvas — drag-to-connect', () => {
  test('connecting a stage to a gateway adds a route and opens the inspector on it', async ({ page }) => {
    ...
    const chipsBefore = await page.locator('prism-workflow-graph [data-prism-transition]').count();
    ...
    const target = await page.locator('prism-workflow-graph [data-prism-gateway-node="await-payment-confirmation"]').boundingBox();
    ...
    await expect
      .poll(() => page.locator('prism-workflow-graph [data-prism-transition]').count())
  ...
  test('connecting stage to stage routes through an auto-created Split gateway', async ({ page }) => {
    ...
    const target = await page.locator('prism-workflow-graph [data-prism-stage-card="payment-complete"]').boundingBox();
    await page.mouse.move(handle.x, handle.y);
    await page.mouse.down();
    ...
    await expect(
      page.locator('prism-workflow-graph [data-prism-gateway="route-from-confirm-payment-received"]')
  ...
  test('read-only canvases expose no connectable handles', async ({ page }) => {
    await gotoStory(page, '/iframe.html?id=workflow-editor-workflow-graph--graph-read-only&viewMode=story');

    const connectable = await page.locator('prism-workflow-graph').evaluate(graphElement => {
      const root = (graphElement as HTMLElement).shadowRoot!;
...
test.describe('Workflow canvas — marquee subgraph copy/paste', () => {
  test('shift-marquee selection copies and pastes a subgraph with remapped keys, undone in one step', async ({ page }) => {
    ...
    const stagesBefore = await page.locator('prism-workflow-graph [data-prism-stage]').count();
    const canvas = await page.locator('prism-workflow-graph .graph-canvas').boundingBox();
    const first = await page.locator('prism-workflow-graph [data-prism-stage-card]').first().boundingBox();
    const second = await page.locator('prism-workflow-graph [data-prism-stage-card]').nth(1).boundingBox();
    ...
    await expect
      .poll(() => page.locator('prism-workflow-graph').evaluate(graphElement =>
    ...
    await expect
      .poll(() => page.locator('prism-workflow-graph [data-prism-stage]').count())
      .toBeGreaterThan(stagesBefore);
    const copies = page.locator('prism-workflow-graph [data-prism-stage*="-copy"]');
