...
test.describe.fixme('Browser-hosted workflow surface: Usability proof', () => {
  test.describe('1. Visual workspace prioritization', () => {
    test('editor workspace is not overwhelmed by host marketing chrome', async ({ page }) => {
      ...
      // Hero/launch card should be visually secondary to the workflow workspace
      ...
      await expect(mainLandmark.locator('prism-workflow-editor')).toBeVisible();
    });
    ...
    test('integration rail does not steal focus from editor workspace', async ({ page }) => {
      ...
      const mainContent = page.locator('#workflow-editor-reference-main');
  ...
  test.describe('2. Swim lane reachability and navigation', () => {
    ...
    test('swim lane horizontal scroll does not break with browser chrome', async ({ page }) => {
      ...
      // BEHAVIORAL REQUIREMENT FOR ISABELLE:
      // If workflow has >5 stages, horizontal scroll should work within editor frame
      ...
      const editorScrolled = await graphCanvas.evaluate((el) => {
        const scrollParent = el.closest('[data-prism-component="workflow-graph"]');
        return scrollParent ? scrollParent.scrollLeft > 0 : false;
      });
      
      // This may be false if the planning workflow is short, but proves containment
      if (editorScrolled) {
  ...
  test.describe('3. Keyboard and screen reader accessibility', () => {
    test('skip link works from browser chrome to editor', async ({ page }) => {
      ...
      // Target: #workflow-editor-reference-main
    ...
    test('tab order flows logically from host chrome to editor', async ({ page }) => {
      ...
      await page.keyboard.press('Tab'); // Launch form (workflow dropdown)
      const workflowDropdown = page.locator('#workflow-key');
      await expect(workflowDropdown).toBeFocused();
    ...
    test('screen reader announces workflow structure in browser host', async ({ page }) => {
      await page.goto(shellUrl('planning'));
  ...
  test.describe('4. Simple editing flow from browser entry point', () => {
    ...
    test('can save workflow from browser-hosted editor', async ({ page }) => {
      await page.goto(shellUrl('planning'));
    ...
    test('can switch workflows from browser host without losing editor state', async ({ page }) => {
      await page.goto(shellUrl('planning'));
      ...
      // BEHAVIORAL REQUIREMENT FOR ISABELLE:
      // Switching workflows via launch form should cleanly reload editor
      // No stale state, no broken references
      
      const workflowDropdown = page.locator('#workflow-key');
      await workflowDropdown.selectOption({ label: /information-request/i });

      const launchButton = page.locator('.launch-button');
      await launchButton.click();

      // Editor should reload with new workflow
      await expect(page.locator('prism-workflow-editor')).toHaveAttribute(
        'data-prism-workflow-loaded',
        'information-request',
        { timeout: 30_000 }
      ...
      // Stages should reflect the new workflow
  ...
  test.describe('5. Browser-specific edge cases', () => {
    ...
    test('editor state persists across browser navigation', async ({ page }) => {
      ...
      // BEHAVIORAL REQUIREMENT FOR ISABELLE:
      // URL should reflect current workflow and API base
      // This allows sharing/bookmarking specific workflows
      
      const url = page.url();
      expect(url).toContain('workflow=planning');
      
      // Navigate away and back
      await page.goto('about:blank');
      await page.goto(url);
      
      // Editor should reload to the same workflow
      await expect(page.locator('prism-workflow-editor')).toHaveAttribute(
        'data-prism-workflow-loaded',
        'planning',
        { timeout: 30_000 }
    ...
    test('editor handles API errors gracefully in browser host', async ({ page }) => {
      // Use an invalid workflow key
      await page.goto(shellUrl('nonexistent-workflow'));
