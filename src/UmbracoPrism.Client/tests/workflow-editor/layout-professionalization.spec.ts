...
test.describe.fixme('Layout Professionalization', () => {
  ...
  test.describe('2. Simplified launch flow', () => {
    ...
    test('launch card is streamlined or removed', async ({ page }) => {
      ...
      // (a) Collapse to a compact workflow switcher (dropdown or tabs)
      ...
      // Target: Workflow selection is utility, not hero action
      ...
      // The "Open workflow" button language is too heavy for a switcher
      await expect(launchButton).toHaveCount(0);
    });

    test('workflow selection is quick-access utility', async ({ page }) => {
      // BEHAVIORAL HOOK REQUEST FOR ISABELLE:
      // If workflow selection remains visible, it should be:
      ...
      //
      // Target: Switching workflows feels like switching tabs, not launching apps
      
      const workflowSelector = page.locator('[data-prism-workflow-selector]');
      
      // OPTIONAL SEMANTIC HOOK: [data-prism-workflow-selector]
      // If Isabelle keeps workflow selection visible, she should add this hook
      // so tests can verify it's present and accessible
      
      if (await workflowSelector.count() > 0) {
  ...
  test.describe('3. Editor surface prioritization', () => {
    ...
    test('editor is the primary mounted experience, not a section within chrome', async ({ page }) => {
      ...
      const editorElement = page.locator('prism-workflow-editor');
      
      // Main should contain the editor as its primary child
      await expect(main.locator('prism-workflow-editor')).toBeVisible();
  ...
  test.describe('4. Keyboard and screen reader accessibility', () => {
    test('skip link jumps directly to editor canvas', async ({ page }) => {
      ...
      // The skip link (#workflow-editor-reference-main) should remain functional
      // and jump to the editor canvas, skipping any minimal utility chrome
      
      const skipLink = page.locator('.skip-link');
      await expect(skipLink).toBeVisible();
      await expect(skipLink).toHaveAttribute('href', '#workflow-editor-reference-main');
      ...
      const main = page.locator('#workflow-editor-reference-main');
      await expect(main).toBeFocused();
    });

    test('tab order flows from utility controls to editor canvas', async ({ page }) => {
      // BEHAVIORAL HOOK REQUEST FOR ISABELLE:
      // If any utility controls remain (workflow switcher, settings), they should:
      ...
      while (tabCount < 5) {
        const currentFocus = await page.evaluate(() => {
          const el = document.activeElement;
          return el?.closest('[data-prism-workflow-outline], [data-prism-component="workflow-graph"]') !== null;
        });
    ...
    test('screen reader announces editor as primary landmark', async ({ page }) => {
      ...
      // Screen reader users should hear: "main, workflow editor" (not nested sections)
      
      const main = page.locator('main[id="workflow-editor-reference-main"]');
  ...
  test.describe('5. Editor functionality preservation', () => {
    ...
    test('graph/list toggle remains functional', async ({ page }) => {
      ...
      // Toggle to list view
      const listToggle = page.locator('prism-workflow-graph').getByRole('button', { name: 'List view' });
      ...
      const graphToggle = page.locator('prism-workflow-graph').getByRole('button', { name: 'Graph view' });
