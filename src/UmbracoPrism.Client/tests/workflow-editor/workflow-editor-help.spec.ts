...
async function dispatchEditorShortcut(
  ...
  await page.locator('prism-workflow-editor').evaluate((element, shortcut) => {
...
test.describe('Workflow editor help and shortcut reference', () => {
  test('help button and F1 open the shortcut guide, and the list stays aligned with the exported shortcut map', async ({ page }) => {
    ...
    for (const group of WORKFLOW_SHORTCUT_GROUPS) {
  ...
  test.fixme('save and redo shortcuts stay discoverable and wired to the host editor commands', async ({ page }) => {
    ...
    await expect(page.locator('[data-prism-toast]')).toContainText('Workflow saved and published.');
  ...
  test('empty workflows show getting-started guidance and still expose help', async ({ page }) => {
    await page.goto(storyUrl('workflow-editor-editor-host--empty-workflow'));
