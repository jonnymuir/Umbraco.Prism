// Executable counterpart of docs/walkthroughs/planning-service-blueprint-editor.md. See .claude/skills/walkthroughs-as-executable-specs/SKILL.md.
...
const WALKTHROUGH_KEY = 'planning-service-blueprint-editor';
const authoredServiceBlueprintRoot = path.resolve(process.cwd(), '../UmbracoPrism.MockBusinessApp/service-blueprint-authored');
const planningServiceBlueprintPath = path.join(authoredServiceBlueprintRoot, 'planning.json');
const provenancePath = path.join(authoredServiceBlueprintRoot, '.provenance');

// URL pattern for the service blueprint editor SPA — stays constant across all steps.
// `service-blueprint-editor.html` is served by MockBusinessApp (Isabelle's Wave 1 foundation deliverable).
const editorUrl = /service-blueprint-editor\.html/;
...
function editorHealthCheck(override: Partial<PageHealthCheck> = {}): PageHealthCheck {
  return {
    url: editorUrl,
    heading: /service-blueprint editor/i,
    bodyMustNotContain: /\b(404|Not Found|An error occurred|Server Error)\b/i,
    ...override,
  };
}

test.describe('Planning Service Blueprint Editor walkthrough', () => {
  test.fixme();
  test.describe.configure({ mode: 'serial' });
  test.setTimeout(12 * 60_000);

  let originalPlanningServiceBlueprint = '';
  let originalProvenanceFiles: string[] = [];

  test.beforeEach(async ({ request }) => {
    await resetServiceBlueprints(request);
    originalPlanningServiceBlueprint = await readFile(planningServiceBlueprintPath, 'utf8');
    originalProvenanceFiles = await readdir(provenancePath).catch(() => []);
  });

  test.afterEach(async () => {
    await writeFile(planningServiceBlueprintPath, originalPlanningServiceBlueprint, 'utf8');
  ...
  //      populate docs/images/walkthroughs/planning-service-blueprint-editor/.
  //   3. Commit the screenshots and update this spec if any selectors drift.
  //
  // See SKILL.md R6 for the screenshot capture workflow.
  // ---------------------------------------------------------------------------

  test('happy path: authoring a planning permission service blueprint with the role-first workspace', async ({ page, appHost }) => {
    await signIn(page);

    // ─── Step 1: Load the reference shell from the business app ────────────────
    // The shell redirects /service-blueprint-editor → /service-blueprint-editor.html?service blueprint=planning
    // and demonstrates the minimal downstream integration surface.
    const serviceBlueprintListResponse = page.waitForResponse(
      response =>
        response.request().method() === 'GET' &&
        response.url().includes('/api/service-blueprint-authoring/service-blueprints') &&
        response.status() === 200
    );
    await page.goto(`${businessAppOrigin}/service-blueprint-editor`);
    await serviceBlueprintListResponse;
    await expect(page).toHaveURL(/\/service-blueprint-editor\.html\?service blueprint=planning(?:&|$)/);
    await expect(page.getByRole('heading', { name: /service-blueprint editor/i })).toBeVisible();
    await expect(page.locator('[data-prism-component="service-blueprint-editor-shell"]')).toHaveAttribute(
      'data-prism-active-service-blueprint',
      'planning'
    );
    await expect(page.locator('prism-service-blueprint-editor')).toHaveAttribute('data-prism-service-blueprint-loaded', 'planning', {
      timeout: 30_000,
    });
    await expect(page.getByRole('combobox', { name: 'Select service blueprint' })).toHaveValue('planning');
    await expect(page.getByRole('alert')).toHaveCount(0);

    // Wait for the service blueprint data to load before asserting page health. The custom-element host
    // reflects data-prism-service-blueprint-loaded="{key}" after the API fetch completes. On slower CI
    // hardware, the page.goto() 'load' event fires before the JS module executes and the
    // async service blueprint fetch finishes, causing the heading check to race and timeout.
    //
    ...
    try {
      ...
      const diagnostics = await page.evaluate(() => {
        const editorElement = document.querySelector('prism-service-blueprint-editor');
        const loadedAttr = editorElement?.getAttribute('data-prism-service-blueprint-loaded') ?? 'element-not-found';
        const bodySnippet = document.body.innerText.substring(0, 500);
        const customElementDefined = !!customElements.get('prism-service-blueprint-editor');
      ...
      console.error('❌ ServiceBlueprint editor readiness timeout. Diagnostics:', JSON.stringify(diagnostics, null, 2));
      throw new Error(`ServiceBlueprint editor failed to load within 30s. State: ${JSON.stringify(diagnostics)}`);
    }

    await step(page, '01-service-blueprint-editor-loaded.png', editorHealthCheck({
      screenshotSelector: '[data-prism-component="service-blueprint-editor-shell"]',
    }), WALKTHROUGH_KEY);
    ...
    // The service blueprint outline should be visible alongside the main canvas at all times.
    // It provides quick navigation to stages and shows the current selection.
    const serviceBlueprintOutline = page.locator('[data-prism-service-blueprint-outline]');
    await expect(serviceBlueprintOutline).toBeVisible({ timeout: 10_000 });
    await expect(serviceBlueprintOutline.locator('[data-prism-outline-stage]')).not.toHaveCount(0);
    ...
    // prism-service-blueprint-graph renders in "graph" mode by default (aria role="application").
    ...
    // without depending on a specific number of lanes in the seed service blueprint.
    ...
    await step(page, '02-graph-view-stages.png', editorHealthCheck({
      screenshotSelector: '[data-prism-component="service-blueprint-graph"]',
    }), WALKTHROUGH_KEY);
    ...
    const outlineDeclarationStage = serviceBlueprintOutline.locator(
    ...
    await expect(serviceBlueprintOutline).toBeHidden();
    ...
    await step(page, '05-collapsed-side-panels.png', editorHealthCheck({
      screenshotSelector: '[data-prism-component="service-blueprint-editor"]',
    }), WALKTHROUGH_KEY);

    await outlineToggle.click();
    await inspectorToggle.click();
    await expect(serviceBlueprintOutline).toBeVisible();
    await expect(stepInspector).toBeVisible();

    // ─── Graph-only contract: no list workspace, canvas owns scrolling ─────────
    await expect(page.locator('prism-service-blueprint-editor').getByRole('button', { name: /List view/i })).toHaveCount(0);
    ...
    const graphScrollState = await page.locator('prism-service-blueprint-graph').evaluate(graphElement => {
    ...
    await expect(page.getByRole('heading', { name: /service-blueprint validation/i })).toBeVisible({ timeout: 5_000 });
    ...
    // ─── Step 9: Simulation tab starts from the service blueprint's initial stage ───────
