// Runs the pure workflow-graph layout checks against the real MockBusinessApp
// seed workflows. Uses Vite in SSR mode so the editor's .js-specifier TS
...
try {
  const mod = await server.ssrLoadModule('/src/workflow-editor/graph/workflow-graph-layout.test.ts');
