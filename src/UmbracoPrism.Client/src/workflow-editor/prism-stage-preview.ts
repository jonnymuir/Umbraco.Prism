...
export class PrismStagePreviewElement extends LitElement {
  ...
  @property({ attribute: false })
  projectedState: ProjectedWorkflowState | null = null;

  @property({ attribute: false })
  outgoingTransitions: ProjectedWorkflowTransition[] = [];
  ...
  render() {
    ...
    return html`
      <section class="preview-shell" aria-labelledby="workflow-stage-preview-title" data-prism-stage-preview>
        <div class="preview-header">
          <div>
            <p class="preview-eyebrow">Preview and runtime format</p>
            <h2 id="workflow-stage-preview-title" class="preview-title">Stage preview</h2>
  ...
  private _renderActions(transitions: ProjectedWorkflowTransition[]): TemplateResult | typeof nothing {
...
function shellLabelFor(state: ProjectedWorkflowState): string {
