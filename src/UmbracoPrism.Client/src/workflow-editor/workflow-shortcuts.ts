export type WorkflowShortcutMatcher = {
  key: string;
  ctrlOrMeta?: boolean;
  shift?: boolean;
  alt?: boolean;
...
export type WorkflowShortcutDefinition = {
  ...
  labels: string[];
  ariaKeys?: string;
  matchers: WorkflowShortcutMatcher[];
};

export type WorkflowShortcutGroup = {
  id: string;
  title: string;
  shortcuts: WorkflowShortcutDefinition[];
};

export const WORKFLOW_SHORTCUT_GROUPS: WorkflowShortcutGroup[] = [
  {
    id: 'editor',
    title: 'Editor-wide',
    shortcuts: [
      {
        ...
        description: 'Saves the current workflow when validation allows it.',
        labels: ['Ctrl/Cmd+S'],
      ...
      },
      {
        ...
        context: 'Anywhere in the editor',
        description: 'Opens the shortcut reference without leaving the workflow.',
        labels: ['F1'],
        ariaKeys: 'F1',
        matchers: [{ key: 'f1' }],
      },
  ...
  },
  {
    ...
    shortcuts: [
      {
        id: 'select-item',
        command: 'Select focused stage or transition',
        context: 'Graph nodes, transition chips, or list rows',
      ...
      },
      {
        ...
        context: 'Focused stage, transition, or list row',
      ...
      {
        id: 'add-transition',
        command: 'Create a transition',
        context: 'Focused stage in graph view',
        description: 'Starts transition authoring from the focused stage.',
        labels: ['T'],
        matchers: [{ key: 't' }],
      },
      {
        ...
        context: 'Focused stage, transition, or list row',
        description: 'Opens stage and transition actions from the keyboard.',
        labels: ['Shift+F10'],
        matchers: [{ key: 'f10', shift: true }],
      },
      {
        id: 'delete-stage-transition',
        command: 'Delete the selected stage or transition',
        context: 'Focused stage, transition, or list row',
        description: 'Opens delete confirmation or removes the focused transition.',
      ...
      },
      {
        ...
        context: 'List workspace',
        description: 'Moves the focused stage earlier or later in the workflow.',
        labels: ['Alt+↑', 'Alt+↓'],
        matchers: [
...
export const WORKFLOW_SHORTCUTS = WORKFLOW_SHORTCUT_GROUPS.flatMap(group => group.shortcuts);

export function findWorkflowShortcut(id: string): WorkflowShortcutDefinition | undefined {
  return WORKFLOW_SHORTCUTS.find(shortcut => shortcut.id === id);
}

export function matchesShortcut(event: KeyboardEvent, shortcut: WorkflowShortcutDefinition): boolean {
