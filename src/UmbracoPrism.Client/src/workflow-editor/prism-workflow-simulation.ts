...
export type WorkflowSimulationStopReason = 'waiting' | 'terminal' | 'no-transitions' | null;

export interface WorkflowSimulationHistoryEntry {
  stageKey: string;
  stageLabel: string;
...
export interface WorkflowSimulationTransitionOption {
...
const STOP_COPY: Record<Exclude<WorkflowSimulationStopReason, null>, string> = {
...
export class PrismWorkflowSimulationElement extends LitElement {
  ...
  @property({ attribute: false })
  transitionOptions: WorkflowSimulationTransitionOption[] = [];
  ...
  @property({ type: String })
  stopReason: WorkflowSimulationStopReason = null;
  ...
  render() {
    ...
    return html`
      <section class="simulation-panel" aria-labelledby="workflow-simulation-title" data-prism-simulation-panel>
        <div class="simulation-header">
          <div>
            <p class="simulation-eyebrow">Preview and simulation</p>
            <h2 id="workflow-simulation-title" class="simulation-title">Path simulation</h2>
        ...
        ${this.initialStage
          ...
          : html`
              <p class="simulation-empty">
                Choose an initial stage before you simulate this workflow.
              </p>
            `}
...
declare global {
  interface HTMLElementTagNameMap {
    'prism-workflow-simulation': PrismWorkflowSimulationElement;
  }
}
