import type { ActionCatalogEntry, AuthoredAction, AuthoredTouchpoint, AuthoredServiceBlueprint, RouteView } from './types.js';
import { stageActions, stageKind, serviceBlueprintGateways, serviceBlueprintTouchpoints } from './types.js';
...
const TERMINAL_STAGE_KINDS = new Set<AuthoredTouchpoint['metadata'] extends never ? never : ReturnType<typeof stageKind>>(['Confirmation']);
...
export function isTerminalStage(stage: AuthoredTouchpoint): boolean {
...
export function serviceBlueprintReachableStageKeys(serviceBlueprint: AuthoredServiceBlueprint): Set<string> {
  const stageKeys = new Set(serviceBlueprintTouchpoints(serviceBlueprint).map(stage => stage.stateKey));
  const gatewayKeys = new Set(serviceBlueprintGateways(serviceBlueprint).map(gateway => gateway.key));
  if (stageKeys.size === 0) {
    return new Set<string>();
  }

  const startStageKey = stageKeys.has(serviceBlueprint.initialTouchpointKey)
    ? serviceBlueprint.initialTouchpointKey
    : serviceBlueprint.touchpoints[0]?.stateKey;
...
export function serviceBlueprintOrphanedStages(serviceBlueprint: AuthoredServiceBlueprint): AuthoredTouchpoint[] {
  return serviceBlueprint.touchpoints.filter(stage =>
    stage.stateKey !== serviceBlueprint.initialTouchpointKey
...
export function serviceBlueprintUnreachableStages(serviceBlueprint: AuthoredServiceBlueprint): AuthoredTouchpoint[] {
  ...
  return serviceBlueprint.touchpoints.filter(stage => !reachable.has(stage.stateKey) && !orphanedKeys.has(stage.stateKey));
}

export function serviceBlueprintDeadEndStages(serviceBlueprint: AuthoredServiceBlueprint): AuthoredTouchpoint[] {
  const orphanedKeys = new Set(serviceBlueprintOrphanedStages(serviceBlueprint).map(stage => stage.stateKey));
  return serviceBlueprint.touchpoints.filter(stage =>
...
export function serviceBlueprintRoutesWithMissingStages(serviceBlueprint: AuthoredServiceBlueprint): RouteView[] {
  const stageKeys = new Set(serviceBlueprint.touchpoints.map(stage => stage.stateKey));
  const gatewayKeys = new Set(serviceBlueprintGateways(serviceBlueprint).map(gateway => gateway.key));
...
function stageLabel(serviceBlueprint: AuthoredServiceBlueprint, stageKey: string) {
  return serviceBlueprint.touchpoints.find(stage => stage.stateKey === stageKey)?.displayName
...
export function validateServiceBlueprint(serviceBlueprint: AuthoredServiceBlueprint, actionCatalog: ActionCatalogEntry[] = []): ServiceBlueprintValidationIssue[] {
  const initialStageExists = serviceBlueprint.touchpoints.some(stage => stage.stateKey === serviceBlueprint.initialTouchpointKey);
  const initialStageIssues = initialStageExists || serviceBlueprint.touchpoints.length === 0
    ? []
    : [{
        ...
        blocking: true,
        location: { kind: 'stage', stageKey: serviceBlueprint.initialTouchpointKey || serviceBlueprint.touchpoints[0]?.stateKey || '' } as const,
        message: serviceBlueprint.initialTouchpointKey
          ? `The serviceBlueprint start stage “${serviceBlueprint.initialTouchpointKey}” is missing. Pick an existing initial stage before you save or simulate this serviceBlueprint.`
          : 'The serviceBlueprint does not have an initial stage yet. Pick one before you save or simulate this serviceBlueprint.',
      }];
  ...
  const unreachableIssues = serviceBlueprintUnreachableStages(serviceBlueprint).map(stage => ({
    ...
    blocking: true,
    location: { kind: 'stage', stageKey: stage.stateKey } as const,
    message: `Stage “${stage.displayName}” is unreachable from the serviceBlueprint start. Add or retarget a route through a gateway so authors can get there.`,
  }));
  ...
  const missingStageRouteIssues = serviceBlueprintRoutesWithMissingStages(serviceBlueprint).map(view => {
    ...
    return {
      ...
      message: missingSource && missingTarget
        ? `Route “${view.action}” is disconnected because both ends are missing. Reconnect it to existing stages before you save or simulate this serviceBlueprint.`
        : `Route “${view.action}” points to a missing ${direction} step “${missingLabel}”. Reconnect it to an existing stage or gateway before you save or simulate this serviceBlueprint.`,
    };
  });

  const stageActionIssues = serviceBlueprint.touchpoints.flatMap(stage =>
