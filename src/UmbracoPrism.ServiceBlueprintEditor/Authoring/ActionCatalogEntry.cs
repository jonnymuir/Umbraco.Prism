...
/// <summary>
/// Design-time descriptor for an action the blueprint editor can offer to authors.
/// Reuses the authored parameter schema contract from issue #55 so catalog metadata and blueprint
/// validation speak the same language.
/// </summary>
public record ActionCatalogEntry
{
    /// <summary>Stable action type key stored in authored blueprint JSON.</summary>
    ...
    /// <summary>Where the action is valid, for example <c>touchpoint.onEntry</c> or <c>transition</c>.</summary>
