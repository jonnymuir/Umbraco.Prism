...
/// <summary>
/// Structural validator for the authored blueprint schema.
/// </summary>
public static class AuthoredServiceBlueprintSchemaValidator
{
        ...
        IActionCatalogProvider? actionCatalogProvider = null)
    {
        ...
        var touchpointKeys = authored.Touchpoints
            .Where(touchpoint => !string.IsNullOrWhiteSpace(touchpoint.TouchpointKey))
            .Select(touchpoint => touchpoint.TouchpointKey)
        ...
        {
            diagnostics.Add(Error("PROJ102", "InitialTouchpointKey is required.", null));
        }

        if (authored.Touchpoints.Count == 0)
        {
            diagnostics.Add(Error("PROJ103", "At least one touchpoint is required.", null));
        }

        foreach (var touchpoint in authored.Touchpoints)
        {
            ValidateTouchpoint(touchpoint, gatewayKeys, queuesByKey, schemaByKey, catalogByType, diagnostics);
        }

        var validGatewayTargets = new HashSet<string>(touchpointKeys, StringComparer.Ordinal);
    ...
    private static void ValidateTouchpoint(
        ...
        List<ProjectionDiagnostic> diagnostics)
    {
        if (string.IsNullOrWhiteSpace(touchpoint.DisplayName))
        {
            diagnostics.Add(Error("PROJ104",
                $"Touchpoint '{touchpoint.TouchpointKey}' must define a title.",
                touchpoint.TouchpointKey));
        }

        if (!string.IsNullOrWhiteSpace(touchpoint.UnknownKindToken))
        {
            diagnostics.Add(Error("PROJ005",
                $"Unknown touchpoint kind '{touchpoint.UnknownKindToken}'. Allowed kinds: " +
                $"{string.Join(", ", ServiceBlueprint.KnownTouchpointKinds)}.",
                touchpoint.TouchpointKey));
        }

        if (string.IsNullOrWhiteSpace(touchpoint.QueueKey))
        {
            if (queuesByKey.Count > 0)
            {
                diagnostics.Add(Error("PROJ153",
                    $"Touchpoint '{touchpoint.TouchpointKey}' must reference a queue.",
                    touchpoint.TouchpointKey));
            }
        }
        else if (!queuesByKey.TryGetValue(touchpoint.QueueKey, out var queue))
        {
            diagnostics.Add(Error("PROJ129",
                $"Touchpoint '{touchpoint.TouchpointKey}' references unknown queue '{touchpoint.QueueKey}'.",
                touchpoint.TouchpointKey));
        }
        else
        {
            ValidateAssignmentCompatibility(touchpoint.TouchpointKey, "Touchpoint", touchpoint.Actor, touchpoint.RoleGates, queue, diagnostics);
        }

        var routeIds = new HashSet<string>(StringComparer.Ordinal);
        var triggerTargets = new HashSet<(string Trigger, string Target)>();
        for (var index = 0; index < touchpoint.Routes.Count; index++)
        {
            var route = touchpoint.Routes[index];

            if (string.IsNullOrWhiteSpace(route.Id))
            {
                diagnostics.Add(Error("PROJ154",
                    $"Route #{index} on touchpoint '{touchpoint.TouchpointKey}' must define an id.",
                    touchpoint.TouchpointKey));
            }
            else if (!routeIds.Add(route.Id))
            {
                diagnostics.Add(Error("PROJ158",
                    $"Touchpoint '{touchpoint.TouchpointKey}' has duplicate route id '{route.Id}'.",
                    touchpoint.TouchpointKey));
            }
            ...
            if (string.IsNullOrWhiteSpace(route.Trigger))
            {
                diagnostics.Add(Error("PROJ155",
                    $"Route '{route.Id}' on touchpoint '{touchpoint.TouchpointKey}' must define a trigger.",
                    touchpoint.TouchpointKey));
            }
            ...
            if (string.IsNullOrWhiteSpace(route.Target))
            {
                diagnostics.Add(Error("PROJ156",
                    $"Route '{route.Id}' on touchpoint '{touchpoint.TouchpointKey}' must define a target.",
                    touchpoint.TouchpointKey));
            }
            else if (!gatewayKeys.Contains(route.Target))
            {
                diagnostics.Add(Error("PROJ157",
                    $"Touchpoint '{touchpoint.TouchpointKey}' route '{route.Id}' must target a gateway, but '{route.Target}' is not a known gateway.",
                    touchpoint.TouchpointKey));
            }
                ...
                && !triggerTargets.Add((route.Trigger, route.Target)))
            {
                diagnostics.Add(Error("PROJ159",
                    $"Touchpoint '{touchpoint.TouchpointKey}' has duplicate route '{route.Trigger}' → '{route.Target}'.",
                    touchpoint.TouchpointKey));
            }
        }
        ...
        for (var routeIndex = 0; routeIndex < gateway.Routes.Count; routeIndex++)
        {
            ...
            else if (!validTargets.Contains(route.Target))
            {
                diagnostics.Add(Error("PROJ150",
                    $"Route '{route.Id}' on gateway '{gateway.GatewayKey}' targets '{route.Target}', which is not a known touchpoint or gateway.",
                    gateway.GatewayKey));
            }
        ...
        foreach (var action in actions)
        {
            if (string.IsNullOrWhiteSpace(action.Type))
            {
                diagnostics.Add(Error("PROJ116",
                    $"A {ownerKind} action in '{ownerKey ?? "blueprint"}' must define a type.",
                    ownerKey));
                continue;
            }
    ...
    {
        ("touchpoint", ActionTiming.OnEntry) => ActionCatalogScopes.TouchpointOnEntry,
        ("touchpoint", ActionTiming.OnExit) => ActionCatalogScopes.TouchpointOnExit,
