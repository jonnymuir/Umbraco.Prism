...
/// engine. Previously, saving via the PUT /mockapp/workflows/{key} endpoint only updated
...
public class WorkflowDefinitionUpdateTests : IDisposable
{
    ...
    private static WorkflowDefinitionFile BuildPaymentDemoSeed(string cardholderLabel) =>
        new()
        {
            ...
            [
                new StepDefinition
                {
                    ...
                    Routes =
                    [
                        new WorkflowRouteDefinition { Id = "route-1", Target = "done", Trigger = "submit" }
                    ]
                },
                new StepDefinition
                {
    ...
    private static void WriteSeedFile(string directory, string key, WorkflowDefinitionFile workflow)
    {
    ...
    private static InputComponent? FindInputByFieldKey(WorkflowDefinitionFile definition, string fieldKey)
    {
