...
/// <summary>
/// Regression coverage for a real bug: <see cref="BusinessAppWorkflowEngine"/> overrides
...
/// tests against the base <c>WorkflowRuntimeEngine</c> passed. This test suite exists specifically
...
public class BusinessAppWorkflowEngineChangeLinkTests : IDisposable
{
    ...
    public BusinessAppWorkflowEngineChangeLinkTests()
    {
        ...
        _engine = new BusinessAppWorkflowEngine(logger.Object, mockEnv.Object, sanitizer.Object);
    }
    ...
    private void SeedWorkflow()
    {
        ...
        var definition = new WorkflowDefinitionFile
        {
            ...
            Transitions =
            [
                new WorkflowTransitionFile { FromState = "how-many-bins", ToState = "fee-gateway", Action = "continue" },
                new WorkflowTransitionFile { FromState = "fee-gateway", ToState = "collection-fee", Action = "" }
            ],
            Metadata = new WorkflowDefinitionMetadata
            {
                AuthoredWorkflowId = new Guid("aaaabbbb-cccc-dddd-eeee-000000000095"),
                Gateways =
                [
        ...
        File.WriteAllText(
            Path.Combine(_testSeedDir, "workflow-seeds", "change-link-business-app-test.json"),
    ...
    public void ChangeLink_ThroughBusinessAppWorkflowEngine_NavigatesBackToTargetState()
    {
        ...
        afterChange.Render!.StateDisplayName.Should().Be("How many bins do you have?",
            "BusinessAppWorkflowEngine's override must delegate change: handling to the (fixed) base " +
