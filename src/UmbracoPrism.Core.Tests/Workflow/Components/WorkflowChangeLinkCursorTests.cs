...
/// <see cref="WorkflowRuntimeEngine.Advance"/> special-cases to jump the instance back to an
...
/// <see cref="WorkflowDefinitionFile.ValidateGatewayRouting"/>), which means <c>instance.Cursors</c>
/// is populated by the time a user reaches a review/summary stage — but the render is built from
/// <c>Cursors</c>, not <c>CurrentState</c>, once any cursor exists
/// (<c>WorkflowRuntimeEngine.FindAccessibleWorkItems</c>). The original "change:" handler only
/// updated <c>CurrentState</c>, so the jump was a silent no-op for any workflow with a gateway in
...
/// </summary>
public class WorkflowChangeLinkCursorTests
{
    ...
    public void ChangeLink_AfterGatewayRoutedStages_NavigatesBackToTargetState()
    {
        ...
        instanceBeforeChange.Cursors.Should().NotBeEmpty(
            "gateway-routed stages populate cursors — this is what made the old CurrentState-only fix a no-op");
    ...
    private static WorkflowDefinitionFile BuildLinearGatewayRoutedDefinition() => new()
    {
