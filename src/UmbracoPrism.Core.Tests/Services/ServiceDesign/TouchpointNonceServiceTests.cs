...
public class TouchpointNonceServiceTests
{
    // ------------------------------------------------------------------ Helpers

    private static TouchpointNonceService BuildService(
        IDistributedCache? cache = null,
        PrismServiceDesignOptions? options = null)
    {
        cache ??= new Mock<IDistributedCache>().Object;
        options ??= new PrismServiceDesignOptions();
        return new TouchpointNonceService(cache, Options.Create(options));
    }
