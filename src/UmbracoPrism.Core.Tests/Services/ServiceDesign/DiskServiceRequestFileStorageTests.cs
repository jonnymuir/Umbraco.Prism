...
public class DiskServiceRequestFileStorageTests : IDisposable
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), $"prism-file-storage-tests-{Guid.NewGuid():N}");
    private readonly DiskServiceRequestFileStorage _storage;

    public DiskServiceRequestFileStorageTests()
    {
        var environment = new Mock<IHostEnvironment>();
        environment.SetupGet(e => e.ContentRootPath).Returns(_root);
        _storage = new DiskServiceRequestFileStorage(environment.Object);
    }
