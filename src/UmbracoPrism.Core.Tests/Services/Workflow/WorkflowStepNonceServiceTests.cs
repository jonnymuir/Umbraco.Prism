...
public class WorkflowStepNonceServiceTests
{
    // ------------------------------------------------------------------ Helpers

    private static WorkflowStepNonceService BuildService(
        IDistributedCache? cache = null,
        PrismWorkflowOptions? options = null)
    {
        cache ??= new Mock<IDistributedCache>().Object;
        options ??= new PrismWorkflowOptions();
        return new WorkflowStepNonceService(cache, Options.Create(options));
    }
    ...
    {
        var cacheMock = new Mock<IDistributedCache>();
        var options = new PrismWorkflowOptions { NonceExpiry = TimeSpan.FromMinutes(30) };
