...
- Keep the **host lookup key** (usually the filename / route key such as `planning`) distinct from the authored workflow's projected `definitionKey` (for example `planning-application`):
  - list/load/save APIs should round-trip on the host key
  - admin/editor pickers should offer the host key the shell can request
  - saving authored edits must write back to the same host key instead of silently switching to `{definitionKey}.workflow.json`
- Quality-gate the three-way contract together:
  1. admin screen advertises an edit link only for workflows backed by an authored source
  2. that link target appears in `/api/workflow-authoring/workflows`
  3. `GET /api/workflow-authoring/workflows/{workflowKey}` loads successfully for the same key
