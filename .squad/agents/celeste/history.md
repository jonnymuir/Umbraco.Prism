...
- 2026-05-31 — Slice D (Celeste hat) — design-doc reframe + decision capture. Deleted `docs/design/workflow-editor-v1/03-umbraco-integration.md` and `04-agentic-surfaces.md` (superseded by the new `docs/guides/umbraco-integration.md` and the agentic-surfaces story being out of scope post-reset). Reframed `docs/guides/workflow-editor-composition.md` as the deeper-dive companion to the new embedding recipe (kept, not redirected — covers the same ground at API-level depth). Updated `docs/design/workflow-editor-v1/02-runtime-projection.md` to the Prism-API framing; simplified `docs/walkthroughs/workflow-administration.md`. Authored close-out decision file `.squad/decisions/inbox/copilot-slice-d-close-out.md` covering: arc complete; `AuthoredTransitionView` debt cleared; pill rendering landed; integrator recipe + walkthrough published; quarantined-spec list (2 describes + 13 individuals) with Slice E TODO; Mabel deferred walkthrough-screenshot note; 4-of-7 walkthroughs left as-is (judged clean of actively-wrong wording); open questions for Jonny (stories that lost two-Split-sharing-a-source pattern, baseline re-cert deferred, simulation-reshape work that lives in Slice E).
