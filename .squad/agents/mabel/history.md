...
- 2026-05-31 — Slice C (frontend, Mabel hat) — walkthrough wording audit, deliberately small. Per the deferred-work spec ("fix actively-wrong sentences only; deeper restructuring is Slice D"), grepped `docs/walkthroughs/` and `docs/design/workflow-editor-v1/02-runtime-projection.md` for stale phrases: "add a transition", "edit the transitions array", "workflow.transitions", "the transitions to", "add transitions". Only one hit in `docs/walkthroughs/payment-demo.md` and on inspection every "transition" reference there is runtime state-change language ("the workflow transitions to processing-payment", "Reviewer transition — Admin panel provides the Complete button") — accurate under both the old and new authoring models, no rewording needed. 02-runtime-projection.md had zero hits for the actively-wrong markers. Confirmed no edits required for this slice; full author-facing sweep (refactoring "transitions" → "routes" in tutorial language, adding gateway-first authoring tutorials) is properly Slice D scope.

- 2026-05-31 — Slice D (Mabel hat) — published the integrator recipe. Created `docs/guides/embedding-the-workflow-editor.md` (~1500 words): install, mount `<prism-workflow-editor>`, wire a workflow-source (`InMemoryWorkflowSource` reference + HTTP shape), persist drafts, validate before publish, with copy-pasteable code blocks. Created `docs/walkthroughs/gateway-first-authoring.md` (~1300 words): Leave Request 5-gateway worked example exercising the per-source feeder-split pattern landed in Slice C. Moved Umbraco integration story from `docs/design/workflow-editor-v1/` to `docs/guides/umbraco-integration.md`. Refreshed `docs/guides/README.md`, `docs/walkthroughs/README.md`, and root `README.md` to point at the new recipe-first ordering. Walkthrough sweep: three of seven (`planning-workflow-editor`, `planning-workflow-complete`, `payment-demo`) had stale "transition" wording corrected to gateway/route; the other four were already clean or used the runtime "transitions to" wording (which is correct under both models). Walkthrough screenshots flagged for a future docs-only pass once pill render stabilises. See `.squad/decisions/inbox/copilot-slice-d-close-out.md`.
