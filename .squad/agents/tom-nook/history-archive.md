...
### 2026-05-04 11:46:55 — Walkthrough Architecture Discovery & Recommendations

**Requested by:** Jonny Muir  
**Scope:** Architecture & discovery pass only — no production code changes.

**Key Findings:**

1. **Walkthrough infrastructure is sound** ✓
   - 9 walkthrough suites (markdown + spec + images) in lockstep
   - All specs gate PRs via `localhost-auth-playwright` CI job
   - SKILL.md enforces executable-specs contract

2. **Six concrete gaps identified:**
   - **Navigation incomplete:** Dashboard shows 2 of 4 workflow types; 2 are invisible without source-code browsing
   - **Admin screen unreachable:** `/admin/workflow` exists (state transitions, JSON editor, instance management) but has no dashboard link; no ops walkthrough possible
   - **Screenshot heights excessive:** `fullPage: true` produces 2500–9447px files (homepage: 9447px ✗); mobile nav leaks into captures adding visual clutter
   - **Push-notifications orphaned:** Spec skipped, markdown written, images directory empty
   - **Workflow movement undocumented:** No walkthrough shows operators how to move instances between states via admin panel
   - **4 seeds vs 9 walkthroughs:** Mismatch suggests incomplete coverage or intentional deferral

3. **Proposed 5-phase implementation slice:**
   - **Phase 1 (Dashboard):** Expose all 4 workflow types; link to `/admin/workflow` (Isabelle + Blathers, 1–2 days)
   - **Phase 2 (Screenshots):** Fixed viewport, hide mobile-nav, regenerate images (Tangy, 2–3 days)
   - **Phase 3 (Admin Walkthrough):** Document state transitions via `/admin/workflow` (Blathers, 2–3 days)
