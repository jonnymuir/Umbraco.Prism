...
- 2026-05-31 — Slice C (frontend, Brewster hat) — MockBusinessApp `/admin/workflow` strip. Now that authoring lives in `/workflow-editor` (Editor Reset) and gateway routing is enforced server-side, the dev-only admin scaffold no longer needs to dual-purpose as a reviewer console and workflow inspector. Trimmed `Program.cs` from ~430 LOC of admin page down to ~155 LOC: kept the instance table (state badge, tenant, ↺ Reset, Reset All) and a flat workflow-definitions list (display name + key/version + ↗ Edit workflow link). Removed: in-page mermaid renderer (`<script type=module>` import of mermaid@10 CDN, `BuildMermaid` helper, `renderCardDiagram`/`renderOpenCardDiagrams`/`expandAllDefs`/`collapseAllDefs` scripts, all `.def-card`/`.def-body`/`.def-diagram`/`.mermaid` styles), per-instance reviewer-action buttons (`ActionDisplay`/`ActionBtnClass`, reviewerTransitions reflection, `/admin/workflow/{id}/action/{action}` endpoint — POST endpoint deleted, not just hidden), JSON modal CSS (`.modal-overlay`/`.modal-box`/`.modal-hdr`/`.modal-ftr`), per-card states/transitions tables, field-groups section (already empty). Kept the `href="/workflow-editor?workflow={authoredWorkflowKey!}"` per-workflow link and the "No editor definition yet" fallback wording (matched against `WorkflowShowcaseShortcutTests.WorkflowAdminScreen_MustLink_BackToEditorReferenceSurfaces`). MockBusinessApp builds; the showcase-shortcut test stays green (811/811). Net effect: a focused admin scaffold that uses Aspire/Umbraco surfaces for their proper jobs (authoring in /workflow-editor, runtime in the public site UI) and only carries dev controls that don't exist elsewhere (instance reset, definition→editor handoff).
