...
### 2026-05-16T17:47:42+01:00 | P1 Prereq Cleared — TestSite Workflow Stub Views Removed

- **Task:** Remove `src/UmbracoPrism.TestSite/Views/workflowPage.cshtml` and `workflowHub.cshtml` stubs that violated `TestSiteViewModelBindingTests` by inheriting raw `ContentModels.*` published models instead of the Core-owned ViewModel.
- **Finding:** The files were already gitignored (`.gitignore` lines 510–511, committed in the V1 design-cycle commit). They existed on disk as untracked/ignored files — likely auto-generated by ModelsBuilder. There was no `git rm` to execute; physical deletion via `rm` was the correct action.
- **No TestSite route-hijacking controller depends on a local view** for either document type — confirmed by grep across all `.cs` files; references to `WorkflowPage`/`WorkflowHub` are for data/seeding only.
- **Build status:** Pre-existing build errors unrelated to this task (`node_modules/govuk-frontend` copy error, MSB3492 cache file transience). Core test project builds and runs cleanly.
- **Test results:** `TestSiteViewModelBindingTests` — 4/4 passed ✅. Full Core suite — 690/690 passed ✅.
- **Git:** No new commit required — the `.gitignore` entries (already committed) are the canonical prevention; physical deletion clears the local environment.
