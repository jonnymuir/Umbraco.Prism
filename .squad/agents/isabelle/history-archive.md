...
# Archived 2026-05-31 and earlier
...
- 2026-05-31T22:26:47+01:00 — Definition-tab UX fix committed (5ee0321). Restored mouse-wheel scrolling by removing `overflow:hidden` on editor host (wheel events now propagate to CodeMirror scroller inside Shadow DOM); wired @codemirror/search + searchKeymap so Cmd/Ctrl+F opens in-editor find panel (Esc to dismiss). Playwright coverage added via `definition-editor-ux.spec.ts` (5/5 tests green: wheel scroll container properly configured, Find open/close, line numbers visible, select-all works). Build ✅, new specs ✅. `definition-tab-sync.spec.ts` excluded from this commit — that file tests bidirectional JSON↔Canvas sync, not the UX fix, and had 2 test failures unrelated to wheel/find work.
- 2026-05-31T22:45:00+01:00 — CodeMirror editors in Shadow DOM need a complete flex layout chain to constrain height: host must be display:flex with flex-direction:column, .cm-editor and .cm-scroller need flex:1 1 0% with min-height:0, and every parent in the chain must have min-height:0 to allow shrinking. Using height:100% or min-height:240px on flex items breaks the constraint. Tab panels must also participate in flex layout (not display:block) for content to be constrained properly. Regression caught by test that asserts scrollHeight > clientHeight, not just checking overflow:auto styles.

---
## 2026-05-31 — Stage UI / GDS regression review (audit only)

**Brief:** Jonny: "the stages used to work really well with the GDS components and things like field sets etc, but now they look very confused."

**Finding:** The stage *components* (fieldset, fields, summary list, accordion) are still GDS-clean. The regression lives in the stage **shells** — `_WorkflowStep-{Question,Review,Completion,StatusTimeline,TaskList,Waiting}.cshtml` — which diverged at commits `40314e2` (PrismComponentTagHelper migration, 2026-04-22) and `7423803` (v2.0 schema, 2026-04-26). Each shell now hand-rolls its own h1, form wrapper, and button-group, with different choices each time. Two shells (`StatusTimeline`, `TaskList`) render `<button type="submit">` with *no enclosing `<form>`* — functional bug, not just visual. The GDS canonical "label/legend as page heading" pattern (`is-page-heading` / `govuk-fieldset__heading`) was deleted entirely; `grep` returns zero hits. The editor's `prism-stage-preview.ts` uses a different heading hierarchy (h3/h4) than the runtime (h1/h2), so authors can't validate runtime structure from the preview.

**Lesson for future UI refactors:** When migrating to a new data model that fans a single render path into multiple shell partials, pull the chrome (page heading, error summary, form, button group) into one shared primitive *first*, then port the bodies. Otherwise the shells diverge silently. Also: anything that used to carry a GDS-significant signal in the old schema (here: `FieldGroup` → "this legend is the page heading") needs to be explicitly re-introduced in the new schema, not just inferred.

**Output:** `.squad/decisions/inbox/isabelle-stage-ui-gds-regression.md` — 7-slice restoration plan, one slice at a time per Jonny's preference. No code changes.
